...
def detect_lines(my_img, edges, threshold1, threshold2, apertureSize,minLineLength,maxLineGap) -> list[Line.Line]:
...
def get_slope_intercepts(lines: list[Line.Line]) -> tuple[list[float], list[int]]:
...
def merge_collinear_lines(lines: list[Line.Line], intercept_margin=20, slope_margin=1) -> list[Line.Line]:
        ...
        if current_slope>0:
            ...
            for j in range(i+1,len(lines)):
                if not lines[j].dealtWith() and abs(current_intercept-lines[j].get_x_intercept()[0])<intercept_margin and lines[j].get_slope()>0 and abs(lines[j].get_slope() - current_slope)<slope_margin:
                    ...
                    lines[j].dealt(True)
            cleanedLines.append(Line.Line(minx1,miny1,maxx2,maxy2))
        else:
            minx1 = min(lines[i].get_points()[0],lines[i].get_points()[2])
            ...
            for j in range(i+1,len(lines)):
                if not lines[j].dealtWith() and abs(current_intercept-lines[j].get_x_intercept()[0])<intercept_margin and lines[j].get_slope()<0 and abs(lines[j].get_slope() - current_slope)<slope_margin:
                    ...
                    lines[j].dealt(True)
            cleanedLines.append(Line.Line(minx1,maxy1,maxx2,miny2))
    return cleanedLines


def detect_lanes(lines: list[Line.Line]) -> list[(Line.Line,Line.Line)]:
